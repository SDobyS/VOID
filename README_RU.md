...
- **API в стиле Raylib**: Чистый и простой игровой цикл (`InitWindow`, `BeginDrawing`, `EndDrawing`, `IsKeyDown` и т.д.)
- **Модульная архитектура**: Чёткое разделение на модули Window, Input, Graphics и Utils
- **Современный графический пайплайн**: OpenGL 4.6 Core Profile с загрузчиком GLAD 2 (автоматическая генерация через CMake)
- **Объектно-ориентированные абстракции**: Инкапсулированные классы `Shader`, `Texture`, `Mesh` (VAO/VBO/EBO) и `VertexArray`
- **2D графический пайплайн**: Специализированный quad-рендерер, оптимизированный под спрайты и пакетную отрисовку
- **Система спрайтов**: Управление текстурами, позициями, размерами, цветами и вращением объектов через удобный интерфейс
- **Ортографическая камера**: Система координат Y-вниз на базе GLM с динамической коррекцией соотношения сторон
- **Современная система шейдеров**: Внешние GLSL-файлы с встроенными uniform-переменными (`u_Model`, `u_ViewProj`, `u_Texture`)
- **Система ввода**: Абстракция клавиатуры и мыши, полностью скрывающая SDL от игрового слоя
- **Загрузка текстур**: Через `stb_image` с автоматическим созданием фиолетово-чёрной шахматной заглушки при ошибке загрузки
- **Дружественность к Pixel Art**: Фильтрация текстур `GL_NEAREST` используется по умолчанию
- **Инструменты разработчика**: Цветная система логирования с временными метками и вывод информации о железе/GPU
- **Зависимости без настройки**: Автоматическое управление зависимостями через CMake FetchContent (без необходимости vcpkg)
- **Продвинутая оконная система**: Настраиваемое создание окон с поддержкой полноэкранного режима, High-DPI, ограничений размера, позиции окна и конфигурации OpenGL-контекста
- **Управление VSync**: Поддержка отключённой, включённой и адаптивной вертикальной синхронизации с контролем swap interval и улучшенным frame pacing
- **Система времени кадра**: Встроенный расчёт Delta Time, FPS-счётчик, отслеживание времени кадра и ограничение FPS
- **Поддержка сглаживания**: Аппаратная настройка MSAA через OpenGL Multisampling
- **Поддержка высокопроизводительных GPU**: Возможность выбора дискретного GPU для ноутбуков с NVIDIA Optimus и AMD PowerXpress
- **Поддержка нескольких мониторов**: Автоматическое определение активного дисплея, частоты обновления и параметров монитора
