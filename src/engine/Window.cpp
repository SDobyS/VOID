...
bool InitWindow(const WindowConfig& config) {
    ...
    if (const SDL_DisplayMode* mode = SDL_GetCurrentDisplayMode(display); mode) { Log::Info("Monitor refresh rate: " + std::to_string(mode->refresh_rate) + " Hz"); }
    else { Log::Warning("Display", "Could not query display mode."); }
    ...
    if (s_VSyncMode != VSyncMode::Disabled && config.fpsLimit > 0) {
        Log::Error("Config Error: VSync and FPS Limit cannot be enabled simultaneously. Disabling FPS Limit.");
        s_FPSLimit = 0;
    } else {
    ...
    if (!s_GLContext) {
        Log::Error("GL Context failed", SDL_GetError());
    ...
    if (s_VSyncMode == VSyncMode::Adaptive) {
        if (!SDL_GL_SetSwapInterval(-1)) {
            Log::Warning("Adaptive VSync unavailable, using normal VSync");
    ...
    Log::Info("Active swap interval: " + std::to_string(interval));
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_EVENT_KEY_DOWN) {
            ...
            if (event.key.scancode == SDL_SCANCODE_F8) {
                s_ShowDebugImGui = !s_ShowDebugImGui;
            }
        }
    }
    BeginImGui();
...
void EndDrawing() {
    ...
    #ifdef _WIN32
        if (s_VSyncMode != VSyncMode::Disabled) {
            DwmFlush();
        }
    #endif
